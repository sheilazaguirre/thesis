<?php
	...
	WHERE c.facultyID='20181009' AND c.status='Active' AND c.semester = '1st Semester' AND c.academicYear=YEAR(NOW())");

	$result2 = mysqli_query($db, "SELECT DISTINCT c.classID, s.subjectCode, c.facultyID, CONCAT(t.dayOfWeek,' ', t.startTime,'-', t.endTime) as timeslot, v.venueCode, c.semester, c.status FROM classes c 
	INNER JOIN subjects s ON c.subjectID = s.subjectID 
	INNER JOIN timeslots t ON c.timeSlotID = t.timeSlotID 
	INNER JOIN venues v ON c.venueID = v.venueID 
	WHERE c.facultyID='20181009' AND c.status='Active' AND c.semester = '2nd Semester' AND c.academicYear=YEAR(NOW())");
